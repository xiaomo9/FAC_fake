...
class DWConv(nn.Module):
    def __init__(self, dim=768):
        ...
        # 使用分组卷积，每个通道单独进行卷积，可以理解为更深入地在每个通道上提取特征
        self.dwconv = nn.Conv2d(dim, dim, kernel_size=3, stride=1, padding=1, bias=True, groups=dim)

    def forward(self, x, H, W):
        B, N, C = x.shape
        # 将输入的特征图从 (B, N, C) 转换为 (B, C, H, W) 的格式，以符合卷积操作的输入需求
        x = x.transpose(1, 2).view(B, C, H, W).contiguous()
        # 执行深度可分离卷积
        x = self.dwconv(x)
        # 将卷积后的结果重新排列回原始的维度 (B, N, C)
...
class ConvolutionalGLU(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        # 激活层
        self.act = act_layer()
        # 第二层全连接层，将特征维度映射到输出维度
        self.fc2 = nn.Linear(hidden_features, out_features)
        # Dropout 层
        self.drop = nn.Dropout(drop)

    def forward(self, x, H, W):
        # fc1 层输出分割为两部分，用于 GLU 的门控
        x, v = self.fc1(x).chunk(2, dim=-1)
        # 经过 DWConv 和激活函数处理后与 v 相乘，实现门控机制
...
class CViT(nn.Module):
    def __init__(self, image_size=224, patch_size=7, num_classes=2, channels=512,
        ...
        self.ggca = GGCA(512,56,56) # channel, h, w
        self.glu = ConvolutionalGLU(1024)  #  C  (B,N,C) 输入通道数C
    ...
    def forward(self, img, mask=None):
        ...
        y = self.patch_to_embedding(y) # 32, 64, 1024
        y = self.glu(y,8,8) # N = H * W 后面两个参数就是H*W
