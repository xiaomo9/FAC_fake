...
def train_gpu(model, criterion, optimizer, scheduler, num_epochs, min_val_loss, min_val_acc):
    ...
    save_path = "./ResVitKan/result"
    if os.path.exists(save_path) is False:
        os.makedirs(save_path)
    # 保存最后epoch训练出的模型
    torch.save(model.state_dict(), "./ResVitKan/result/KanVitRes50_deepfake_last1.pth")
    ...
    with open('./ResVitKan/result/KanVitRes34_deepfake_v1.pkl', 'wb') as f:
    ...
    list.to_csv('./ResVitKan/result/KanVitRes34_deepfake_v1.csv', index=False)

    state = {'epoch': num_epochs + 1,
             'state_dict': model.state_dict(),
             'min_loss': epoch_loss}
    torch.save(state, './ResVitKan/result/KanVitRes34_deepfake_v1.pth')
    test(model)
