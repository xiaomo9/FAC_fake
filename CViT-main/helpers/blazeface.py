...
class BlazeFace(nn.Module):
    """MediaPipe 中的 BlazeFace 人脸检测模型。
    
    MediaPipe 的版本比论文中的版本更简单;
    它不使用“双”BlazeBlocks。

    因为我们不会训练这个模型，所以它不需要有
    BatchNorm 层。这些已经被“折叠”到转换中TFLite的权重。

    转换为 PyTorch 相当简单，但有TFLite 和 PyTorch 在处理方式上的一些细微差异
    在 STRIDE 2 的 conv 图层上填充。

    此版本适用于批处理，而 MediaPipe 版本只能一次处理单个图像。
